...
export function FloatingEditor() {
  ...
  const loadRadioStations = async () => {
    ...
    try {
      ...
      if (stations.length === 0) {
        toast.error('No radio stations available');
        return;
      }
      
      setRadioStations(stations);
      toast.success(`Loaded ${stations.length} quality radio stations`);
    } catch (error) {
  ...
  const togglePlay = async () => {
    ...
    if (isPlaying) {
      ...
      try {
        ...
        toast.success(`Playing: ${currentStation.name}`);
      } catch (error) {
        const errorMsg = error instanceof Error ? error.message : 'Unknown error';
        console.error('Radio play error:', errorMsg);
        toast.error('Failed to play station - trying next one');
  ...
  const changeStation = (direction: 'next' | 'prev') => {
    ...
    if (isPlaying && audioRef.current) {
      ...
      toast.success(`Switched to ${radioStations[newIndex].name}`);
    }
  };
  ...
  const handleSendMessage = async () => {
    ...
    setTimeout(() => {
      const responses = [
        ...
        "Looking for inspiration? Check out the Community page to see what others have created!",
        "Need help finding a tattoo artist? I can guide you through our artist finder feature."
      ];
            ...
            <div className="p-4 border-b border-border/50 space-y-3">
              ...
              <audio
                ...
                onError={(e) => {
                  ...
                  toast.error('Stream error - trying next station');
                  changeStation('next');
                }}
              />
            </div>
