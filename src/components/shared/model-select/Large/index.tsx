...
export function LargeModelOption({ id, label, badge, selected, onSelect, className, disabled, inkCost, estimatedSeconds, locked, lockReason }: BaseModelOptionProps) {
  return (
    <button
    ...
    >
      {badge && (
        <span className="absolute -top-2 -right-2 rounded-full bg-[#57f1d6]/20 text-[#57f1d6] border border-[#57f1d6]/40 px-2 py-0.5 text-[10px] font-semibold tracking-wide">
          {badge}
        </span>
      )}
      {locked && (
